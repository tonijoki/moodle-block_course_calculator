define(['core/str'], function(Str) {
    ...
    const isEntryCharacter = (value) => /^[0-9.,]$/.test(value);
    ...
    const parseDisplayValue = (text) => Number(canonicalNumberString(text || "0"));
    ...
    const renderHistory = (historyNode, historyItems) => {
        historyNode.innerHTML = historyItems.map((item) =>
            '<div class="advanced-calculator__history-item" tabindex="0"><div class="advanced-calculator__history-expression">' +
            ...
            switch (action) {
                ...
                case "angle-mode":
                    ...
                    toggles.forEach((toggle) => {
                        toggle.classList.toggle("is-active", toggle.getAttribute("data-value") === value);
                    });
                    updateError(error, "");
                    focusInput(input);
                    break;
        ...
        const applyEquals = () => {
            try {
                ...
                if (state.lastOperator !== null && state.lastOperand !== null) {
                    ...
                    const expression = formatResult(left, locale, true) + state.lastOperator + formatResult(state.lastOperand, locale, true);
                    setDisplayNumber(result);
                    operation.textContent = expression + " =";
                    pushHistory(expression, formatResult(result, locale, true));
                    state.accumulator = result;
